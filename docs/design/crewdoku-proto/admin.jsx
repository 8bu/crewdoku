/* Crewdoku — admin configuration: shift definitions, rules, teams, solver weights.
   Validation is computed live so infeasible configs warn immediately.
   Permissions section intentionally absent — single-user app, no roles. */
const AdminSurface = (() => {
  ...
  const SECTIONS = [
    ...
    /* Role permissions intentionally removed — Crewdoku is single-user (one planner, no accounts) */
  ];
  ...
  function EmployeeModal({ emp, onSave, onClose }) {
    ...
    const [eligibleShiftIds, setEligibleShiftIds] = useState(
      emp.eligibleShiftIds || SF.SHIFTS.map(s => s.code)
    );
    ...
    const save = () => onSave({ ...emp, name: name.trim() || emp.name, depts, home, prefs, eligibleShiftIds });
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center"
        ...
        <div className="rounded-[2px] border border-bd flex flex-col"
          ...
          <div className="overflow-auto flex-1 p-4 space-y-4">
            ...
            {/* Eligible shifts — H6 */}
            <div>
              <div className="text-2xs font-semibold uppercase tracking-wider text-dim mb-1.5">Eligible shifts <span className="font-mono normal-case font-normal text-faint">(H6)</span></div>
              <p className="text-2xs text-faint mb-2 leading-snug">
                Narrow which shifts this employee can be assigned. Defaults to all shifts.
                Uncheck to restrict. “Never” in preferences is a hard block regardless.
              </p>
              <div className="flex flex-wrap gap-x-5 gap-y-2">
                {SF.SHIFTS.map(s => (
                  <label key={s.code} className="flex items-center gap-2 text-xs cursor-pointer select-none">
                    <input type="checkbox"
                      checked={eligibleShiftIds.includes(s.code)}
                      onChange={e => setEligibleShiftIds(prev =>
                        e.target.checked ? [...prev, s.code] : prev.filter(x => x !== s.code)
                      )} />
                    <span className="font-mono text-2xs px-1.5 py-0.5 border rounded-[2px]"
                      style={{
                        background:  "var(--sh-" + s.code + "-bg)",
                        color:       "var(--sh-" + s.code + "-fg)",
                        borderColor: "var(--sh-" + s.code + "-bd)",
                      }}>{s.code}</span>
                    {s.name}
                  </label>
                ))}
              </div>
              {eligibleShiftIds.length === 0 && (
                <p className="text-2xs mt-1.5" style={{ color: "var(--st-crit)" }}>
                  ⚠ No eligible shifts — this employee cannot be scheduled.
                </p>
              )}
            </div>
  ...
  function TeamsConfig({ employees, setEmployees }) {
    ...
    const addSingle = () => {
      ...
      setEmployees(prev => [...prev, {
        ...
        prefs: { ...DEFAULT_PREFS },
        eligibleShiftIds: SF.SHIFTS.map(s => s.code),
      }]);
    ...
    const addMultiple = () => {
      ...
      const parsed = lines.map(line => {
        ...
        return { name, dept: deptObj.id, deptName: deptObj.name, depts: [deptObj.id], home: shiftIdx >= 0 ? shiftIdx : 2, prefs: { ...DEFAULT_PREFS }, eligibleShiftIds: SF.SHIFTS.map(s => s.code) };
      }).filter(Boolean);
      setEmployees(prev => [
        ...prev,
      ...
      if (t.startsWith("[") || t.startsWith("{")) {
        try {
          ...
          const data = arr.map(r => {
            ...
            const shiftObj = SF.SHIFTS.find(s => s.code === shiftRaw);
            return { name, dept: deptObj.id, deptName: deptObj.name, depts: [deptObj.id], home: shiftObj ? SF.SHIFTS.indexOf(shiftObj) : 2, prefs: { ...DEFAULT_PREFS }, eligibleShiftIds: SF.SHIFTS.map(s => s.code) };
          }).filter(Boolean);
