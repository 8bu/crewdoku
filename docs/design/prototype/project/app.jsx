...
const initialState = {
  ...
  emptySchedule: false,    // true after onboarding — board blank until first solve
  locale:        "en",
  dbLoaded:      false,
};
...
function reducer(state, a) {
  switch (a.type) {
    ...
    case "SET_CELL": {
      ...
      return { ...state, overrides, emptySchedule: false };
    }
    case "MOVE_CELL": {
      const overrides = new Map(state.overrides);
    ...
    case "SET_EMPTY_SCHEDULE": return { ...state, emptySchedule: a.value };
    case "DIFF_DECIDE": {
    ...
    case "LOAD_CUSTOM":  return {
      ...
      overrides:     new Map(),
      diff:          null,
      emptySchedule: true,   // start blank — guide user to run solver
    };
    case "RESET":        return {
      ...initialState,
      dbLoaded: true,
    ...
    case "DB_LOAD":      return {
      ...
      emptySchedule: a.data.emptySchedule !== undefined ? a.data.emptySchedule : state.emptySchedule,
      dbLoaded:      true,
    };
...
function Sidebar({ app, dispatch, isDataLoaded }) {
  ...
  return (
    <aside className="w-48 shrink-0 border-r border-bd bg-surface flex flex-col select-none overflow-hidden">
      ...
      {/* locale switcher — i18n, translations wired up in future release */}
      <div className="border-t border-bd px-2.5 py-2 shrink-0">
        <p className="text-2xs font-semibold uppercase tracking-widest px-0.5 pb-1.5" style={{ color: "var(--text-faint)" }}>Language</p>
        <SelectBox
          value={app.locale || "en"}
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(() => {
      DB.set("cw_state_v1", {
        ...
        emptySchedule: app.emptySchedule,
      });
    }, 800);
    return () => clearTimeout(saveTimer.current);
  }, [app.dataMode, app.locale, app.overrides, app.emptySchedule, app.dbLoaded]);
    ...
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <TweaksPanel>
        ...
        <TweakToggle
          ...
          value={app.emptySchedule}
          onChange={v => dispatch({ type: "SET_EMPTY_SCHEDULE", value: v })}
        />
      </TweaksPanel>
    </div>
  );
