/* ShiftForge demo data + deterministic solver simulation. Exposes window.SF */
window.SF = (function () {
  "use strict";
  ...
  const SOFT = [
    { id: "S1", name: "Night-shift fairness",     weight: 8, blurb: "Spread Night shifts evenly across qualified staff over a 4-week window." },
    { id: "S2", name: "Preference satisfaction",  weight: 6, blurb: "Honour member preference profiles where possible." },
    { id: "S3", name: "Minimal changes",          weight: 4, blurb: "Stay close to the current published schedule." },
    { id: "S4", name: "Weekend rotation",         weight: 5, blurb: "No one works 3 weekends in a row." },
    { id: "S5", name: "Sequence consistency",     weight: 3, blurb: "Avoid ping-ponging between very different start times in one week." },
  ];
  ...
  function workloadHistogram(weekOffset, getShift) {
    const buckets = [{ label: "<24h", n: 0 }, { label: "24h", n: 0 }, { label: "30h", n: 0 }, { label: "36h", n: 0 }, { label: ">40h", n: 0 }];
    for (let i = 0; i < 100; i++) {
      const h = weekHours(i, weekOffset, getShift);
      if (h < 24) buckets[0].n++;
      else if (h < 30) buckets[1].n++;
      else if (h < 36) buckets[2].n++;
      else if (h <= 40) buckets[3].n++;
      else buckets[4].n++;
    }
    return buckets;
  }
  ...
  return {
    ...
    weekHours, workloadHistogram, mod,
  };
})();
