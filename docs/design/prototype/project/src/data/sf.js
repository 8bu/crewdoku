/* ShiftForge demo data + deterministic solver simulation. */
...
function baseAssign(i, absDay) {
  if (!inHorizon(absDay)) return undefined
  ...
  if (dow === off1 || dow === off2) return null
...
/* ---- Requests ---- */
const REQUESTS = [
  { id: 'R-1041', empIdx: 12, type: 'dayoff', title: 'Day off',     detail: 'Thu Jun 18 — medical appointment', absDay: 3,  submitted: 'Jun 10', status: 'pending',
    ripple: { level: 'crit', text: 'Approving leaves Night at 9/10 on Thu Jun 18. Nearest fix: solver can move K. Lindqvist (E→N), creating 1 soft penalty (S1 +4).' } },
  { id: 'R-1042', empIdx: 27, type: 'swap',   title: 'Shift swap',  detail: 'Fri Jun 19 · Late ⇄ Early with Dalia Hale', absDay: 4, swapWith: 33, consent: 'accepted', submitted: 'Jun 10', status: 'pending',
    ripple: { level: 'warn', text: 'Swap gives B. Okafor only 9h rest before Sat Early (min 11h, H3). Solvable: solver absorbs with 1 reassignment on Sat.' } },
  { id: 'R-1043', empIdx: 55, type: 'pref',   title: 'Preference change', detail: 'Recurring: avoid Night shifts on Mondays', absDay: null, submitted: 'Jun 09', status: 'pending',
    ripple: { level: 'ok',   text: 'No impact on the current week. Projected fairness drift −0.4 over 4 weeks — within tolerance.' } },
  { id: 'R-1044', empIdx: 64, type: 'sick',   title: 'Unplanned absence', detail: 'Fri Jun 12 — sick day', absDay: -3, submitted: 'Jun 12 06:14', status: 'pending',
    ripple: { level: 'crit', text: 'Swing drops to 15/16 today. T. Mercer is qualified, rested, and at 24h this week — 1-click backfill available.' } },
  { id: 'R-1045', empIdx: 38, type: 'swap',   title: 'Shift swap',  detail: 'Sun Jun 21 · Swing ⇄ Mid with Petra Nakamura', absDay: 6, swapWith: 41, consent: 'awaiting', submitted: 'Jun 11', status: 'pending',
    ripple: { level: 'ok',   text: 'No constraint impact. Both qualified; rest windows unaffected.' } },
  { id: 'R-1046', empIdx: 71, type: 'dayoff', title: 'Day off',     detail: 'Sat Jun 20', absDay: 5, submitted: 'Jun 11', status: 'pending',
    ripple: { level: 'warn', text: 'Weekend Early lands exactly at minimum (14/14). Approvable, but zero slack for sick calls on Sat.' } },
  { id: 'R-1038', empIdx: 12, type: 'swap',   title: 'Shift swap',  detail: 'Tue Jun 09 · Mid ⇄ Early with Jana Visser', absDay: -6, submitted: 'Jun 05', status: 'approved', decidedBy: 'M. Fischer', ripple: { level: 'ok', text: 'No constraint impact.' } },
  { id: 'R-1036', empIdx: 12, type: 'dayoff', title: 'Day off',     detail: 'Mon Jun 08', absDay: -7, submitted: 'Jun 03', status: 'rejected', reason: 'Coverage: two approved absences already on Jun 08 (H1).', decidedBy: 'M. Fischer' },
]
...
const SOFT = [
  { id: 'S1', name: 'Fair night shifts',        weight: 8 },
  { id: 'S2', name: 'Respect people\'s wishes', weight: 6 },
  { id: 'S3', name: 'Keep things stable',       weight: 4 },
  { id: 'S4', name: 'Share the weekends',       weight: 5 },
  { id: 'S5', name: 'Smooth shift changes',     weight: 3 },
]

/* ---- Violation checks (real) ---- */
...
function checkViolations(i, absDay, newCode, getShift) {
  ...
  if (newCode) {
    ...
    if (prev) {
      ...
      if (rest < RULES.minRest) out.push({ rule: 'H3 · MIN_REST_11H', text: 'Only ' + rest + 'h rest after ' + shift(prev).name + ' (ends ' + hh(shift(prev).end) + ').' })
    }
    const next = getShift(i, absDay + 1)
    if (next) {
      const rest = (absDay + 1) * 24 + shift(next).start - (absDay * 24 + shift(newCode).end)
      if (rest < RULES.minRest) out.push({ rule: 'H3 · MIN_REST_11H', text: 'Only ' + rest + 'h rest before ' + shift(next).name + ' on ' + dayLabel(absDay + 1) + '.' })
    }
...
/* ---- Solver proposal (simulated diff) ---- */
const CHANGE_NOTES = [
...
function makeProposal(pinsSet) {
  const changes = []
  for (let i = 0; i < 100 && changes.length < 26; i++) {
...
/* ---- Infeasible scenario ---- */
const INFEASIBLE = {
  summary: 'The solver proved no schedule can satisfy all hard constraints for the week of Jun 15.',
  core: [
...
function workloadHistogram(weekOffset, getShift) {
  const buckets = [
    { label: '<24h', hrs: 0,  count: 0 },
    { label: '24h',  hrs: 24, count: 0 },
    { label: '30h',  hrs: 30, count: 0 },
    { label: '36h',  hrs: 36, count: 0 },
    { label: '>40h', hrs: 48, count: 0 },
  ]
  for (let i = 0; i < 100; i++) {
...
const SF = {
  SHIFTS, SHIFT_IDX, shift, shiftTime, shiftHours, hh,
  DEPTS, EMPLOYEES, initials,
  DOW, dateOf, dayLabel, dayLong, isWeekend, inHorizon, HORIZON,
  baseAssign, PINS, REQUESTS, HARD, SOFT, RULES,
