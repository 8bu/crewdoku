/* MemberSurface — self-service schedule, swaps, preferences */
import { useState, useMemo, useCallback } from 'react'
import SF from '../../data/sf'
import {
  cx, Btn, Badge, ShiftChip, SelectBox, Modal, Panel, EmptyState, Banner,
} from '../../components/ui'
import PageHeader from '../../components/PageHeader'
import Board from '../board/Board'

export default function MemberSurface({ app, dispatch }) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0" data-screen-label="Employee">
      <PageHeader title={pg.title} subtitle={pg.sub}
        actions={view === 'swaps' && incomingSwaps.length > 0 && <Badge tone="warn">{incomingSwaps.length} incoming</Badge>} />
      <div className="flex-1 overflow-auto">
        ...
        {view === 'team'     && <Board app={app} dispatch={dispatch} />}
      </div>
    </div>
  )
}
...
function MySchedule({ app, dispatch, empIdx, me, getShift }) {
  ...
  const teammates = useMemo(() => SF.EMPLOYEES.filter(e => e.dept === me.dept && e.i !== empIdx), [me.dept, empIdx])

  const takeDayOff = (d) => dispatch({ type: 'SET_CELL', key: empIdx + '|' + d, code: null, viol: [] })
    ...
    <div className="p-3 max-w-lg space-y-2">
      ...
      <div className="border border-bd bg-surface rounded-[2px] overflow-hidden">
        {days.map(d => {
          ...
          return (
            <div key={d} className="flex items-center gap-2.5 px-2.5 border-b border-[var(--grid-line)] last:border-b-0"
              ...
              {code ? <ShiftChip code={code} time /> : <span className="text-xs text-faint italic">Day off</span>}
              <span className="flex-1"></span>
              ...
              {inH && !code && !pinned && <span className="text-2xs text-faint">rest day</span>}
            </div>
          )
        })}
      </div>
      ...
      {swapDay !== null && (
        <SwapModal
          ...
          me={me} empIdx={empIdx} teammates={teammates} getShift={getShift}
    ...
    <Modal title={'Propose swap · ' + SF.dayLong(absDay)} onClose={onClose} width={380}
      footer={
        <>
          ...
          <Btn variant="primary" disabled={!canSwap} onClick={() => onPropose(Number(targetIdx), targetShift)}>
            Send swap request
          </Btn>
        </>
      }>
      <div className="space-y-2.5">
        ...
        {target && !targetShift && <Banner level="warn">{target.name.split(' ')[0]} is off that day — can't swap a rest day.</Banner>}
      ...
      <div className="px-2.5 py-2 border-b border-[var(--grid-line)] last:border-b-0">
        ...
        {sw.status === 'pending' && (
          <div className="flex gap-1.5 mt-1.5">
            ...
            {!incoming && <Btn variant="ghost" onClick={() => dispatch({ type: 'SWAP_CANCEL', id: sw.id })}>Cancel request</Btn>}
          </div>
        )}
      </div>
    )
    ...
    <div className="p-3 grid gap-3 items-start" style={{ gridTemplateColumns: '1fr 1fr', maxWidth: '760px' }}>
      <Panel title={'Incoming · ' + incomingSwaps.length} pad={false}>
        {incomingSwaps.length === 0
          ? <EmptyState glyph="◇" title="No incoming swaps" body="When a teammate proposes a swap with you, it appears here." />
    ...
    <div className="p-3 max-w-xl space-y-3" data-screen-label="Employee / Preferences">
      ...
      <Panel title={'Preferences · ' + mine.length} pad={false}>
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-2 border-t border-bd bg-raised flex-wrap">
          ...
          <Btn onClick={() => dispatch({ type: 'PREF_ADD', pref: { id: 'p' + Date.now(), empIdx, kind, shift, scope } })}>+ Add</Btn>
        </div>
      </Panel>
    </div>
  )
