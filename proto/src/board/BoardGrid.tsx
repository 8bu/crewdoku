...
import type { RelaxationOption, ScheduleMap, SolveOptions, SolveRules } from '../engine/types'
...
export function BoardGrid({ periodId, initial }: BoardGridProps) {
  ...
  const runSolveRef = useRef<(options?: SolveOptions) => ReturnType<typeof solve>>(() =>
    Promise.resolve({ status: 'solved', schedule: new Map() }),
  )
  const runSolveStable = useCallback((options?: SolveOptions) => runSolveRef.current(options), [])
  ...
  const generateFlow = useGenerateFlow(runSolveStable, onSolvedStable)

  // Ticket 13: a relaxation rebuilds these rules before the next solve. A
  // ref, not state, for the same reason as `runSolveRef`/`onSolvedRef` above
  // — `applyRelaxation` calls `generateFlow.generate()` right after updating
  // this, and that next solve must see the loosened rules immediately, not
  // after a render round-trip.
  const rulesRef = useRef<SolveRules>({
    coverage: coverageTable,
    minRestHours: solveSettings.hardRules.minRestHours,
    maxHoursPerWeek: solveSettings.hardRules.maxHoursPerWeek,
    enabled: solveSettings.hardRules.enabled,
  })
  useEffect(() => {
    rulesRef.current = {
      coverage: coverageTable,
  ...
  }, [coverageTable, solveSettings.hardRules])
  ...
  runSolveRef.current = (options) => {
    ...
    return solve({
      // Ticket 16: a removed person is never handed anything new — their
      // existing schedule entries above still carry through untouched
      // (`stubSolve`'s `fillSchedule` only overwrites keys for people in
      // `org.people`), so this is the one place "remove" is more than cosmetic.
      org: { teams: teams, people: people.filter((p) => !p.removed), shifts },
      rules: rulesRef.current,
      period: { dates: data.dates },
      schedule,
      options,
    })
  }
  ...
  const applyRelaxation = useCallback(
    (option: RelaxationOption) => {
      rulesRef.current = option.relax(rulesRef.current)
      generateFlow.generate({ delayMs: 4000 })
    },
    [generateFlow],
  )
  ...
  const proposalChangeMessages = useMemo(() => {
    ...
    for (const change of pendingProposal.changes) {
      map.set(assignmentKey(change.personId, change.dateIso), `was: ${shiftLabel(change.from)}`)
    }
    return map
  }, [pendingProposal])
    ...
    const showForTarget = (target: EventTarget | null) => {
      ...
      if (fairHeaderEl) {
        ...
        setTip({
          ...
          message: 'Violations with no cell on the board',
          visible: true,
          below: true,
          kind: 'violation',
        })
    ...
    <div className="cd-board-shell relative flex h-full min-h-0">
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        {settingsDirty && hasSchedule && !pendingProposal && generateFlow.state.phase === 'idle' && (
          <div className="flex items-center gap-2.5 border-b border-warning/40 bg-warning/15 px-3.5 py-[7px] text-xs text-base-content">
            ...
            <span>Rules changed since the last Generate — this schedule may be out of date.</span>
          </div>
        )}
        <div
          ref={scrollerRef}
                  ...
                  teamPeople.map((person) => (
                    <Fragment key={person.id}>
                      ...
                      <div
                        ...
                        role="button"
                        tabIndex={0}
                        aria-label={`Open ${person.name}'s details${person.removed ? ' (removed)' : ''}`}
      ...
      {importOpen && (
        ...
        >
          ...
          >
            <div className="mb-4">
              <h2 className="text-lg font-semibold tracking-tight text-base-content">Import a schedule</h2>
              <p className="mt-1 text-sm text-base-content/60">
                Paste or upload a CSV of an actual schedule — people, teams, and the assignments it lists are all created at
                once.
              </p>
            </div>
            <ScheduleImportPanel
