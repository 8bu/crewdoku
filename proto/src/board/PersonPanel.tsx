...
import type { BoardDate, Person, ShiftCode, ShiftDef, Team } from './mockBoard'
...
function dateLabel(iso: string, dateByIso: Map<string, BoardDate>): string {
  const date = dateByIso.get(iso)
  if (!date) return iso
  return `${WEEKDAY_SHORT[date.weekday]} ${date.monthShort} ${date.dayOfMonth}`
}

function prefLabel(wants: ShiftCode[], avoids: ShiftCode[]): string {
  const parts: string[] = []
  if (wants.length > 0) parts.push(`wants ${wants.join(', ')}`)
  if (avoids.length > 0) parts.push(`avoids ${avoids.join(', ')}`)
  return parts.length > 0 ? parts.join(' · ') : 'no preference set'
}
  ...
  return (
    ...
    >
      <div className="sticky top-0 z-10 flex items-start justify-between gap-2 border-b border-base-300 bg-base-100 px-4 py-3.5">
        <div>
          <div className="text-sm font-semibold text-base-content">
            {person.name}
            {person.removed && (
              <span className="badge badge-error badge-outline badge-xs ml-1.5 align-middle">Removed</span>
            )}
          </div>
          <div className="mt-0.5 text-xs text-base-content/50">{team.name}</div>
        </div>
        <button type="button" className="btn btn-ghost btn-xs btn-circle text-base-content/50" onClick={onClose} aria-label="Close">
          ✕
        </button>
      </div>
      ...
      <div className="flex flex-col gap-[18px] p-4">
        <section>
          <h3 className={heading}>This period</h3>
          <div className="flex gap-2.5">
            ...
            <div className="flex-1 rounded-lg border border-base-300 bg-base-200 px-2.5 py-2">
              ...
              <span className="mt-0.5 block text-2xs text-base-content/50">Nights</span>
            </div>
            <div className="flex-1 rounded-lg border border-base-300 bg-base-200 px-2.5 py-2">
              <span className="font-mono text-lg font-semibold text-base-content">{fairness.weekends}</span>
              <span className="mt-0.5 block text-2xs text-base-content/50">Weekends</span>
            </div>
          </div>
        </section>

        <section>
          <h3 className={heading}>Eligible shifts</h3>
          ...
          <p className={hint}>Click to toggle. Dimmed = not eligible for that shift.</p>
        </section>

        <section>
          <h3 className={heading}>Preferences</h3>
          ...
          <p className="mb-2.5 mt-1.5 text-2xs text-base-content/50">
            Team default: {prefLabel(team.wants, team.avoids)}
          </p>
          <div className="mb-2 flex items-center gap-2.5">
            <span className="w-[52px] flex-none text-2xs text-base-content/50">Wants</span>
            <div className={chipsRow}>
              {shifts.map(({ code }) => (
                <button
                  key={code}
          ...
          <div className="flex items-center gap-2.5">
            <span className="w-[52px] flex-none text-2xs text-base-content/50">Avoids</span>
            <div className={chipsRow}>
              {shifts.map(({ code }) => (
                <button
                  key={code}
        ...
        <section>
          <h3 className={heading}>Recurring unavailability</h3>
          <div className={chipsRow}>
          ...
          <p className={hint}>Every week, regardless of rotation.</p>
        </section>

        <section>
          <h3 className={heading}>Time off</h3>
          {timeOff.length === 0 ? (
            <p className={hint}>None booked this period.</p>
          ) : (
              ...
              {timeOff.map((iso) => (
                ...
                >
                  ...
                  <button
                    ...
                    aria-label={`Remove time off on ${dateLabel(iso, dateByIso)}`}
                  >
                    ✕
                  </button>
                </li>
