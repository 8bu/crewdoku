...
import type { Assignment, Person } from '../board/mockBoard'
...
const STATUS_WORD: Record<string, { word: string; cls: string }> = {
  short: { word: 'Short', cls: 'text-[color:var(--viol)]' },
  over: { word: 'Over', cls: 'text-[color:var(--over)]' },
  ok: { word: 'OK', cls: 'text-[color:var(--ok)]' },
}
...
export function CoveragePopover({ cell, rect, allPeople, getAssignment, onClose }: CoveragePopoverProps) {
  ...
  const status = cell.status ? STATUS_WORD[cell.status]! : null
  ...
  return (
    ...
    >
      <div className="mb-1 flex items-baseline justify-between gap-2">
        ...
        {status && <span className={`text-2xs font-semibold uppercase ${status.cls}`}>{status.word}</span>}
      </div>
      <div className="mb-1.5 text-2xs text-[color:var(--text-dim)]">
        {cell.count} on duty
        {cell.status !== null && ` · needs ${cell.min}${cell.max === Infinity ? '+' : `–${cell.max}`}`}
        {cell.status === 'short' && ` · ${freeCount} eligible & free`}
      </div>
      {cell.people.length > 0 ? (
        ...
        <div className="mb-1.5 py-1 text-xs text-[color:var(--text-faint)]">No one is on this shift.</div>
      )}
      <button type="button" className="btn btn-primary btn-xs w-full" onClick={showOnBoard}>
        {cell.status === 'short' ? `Show ${freeCount} who could fix it` : 'Show on board'}
      </button>
    </div>
  )
}
