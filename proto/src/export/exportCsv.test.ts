...
describe('exportCsv', () => {
  ...
  it('generates exact grid CSV for 2 teams + 1 unassigned person with blank OFF cells', () => {
    const people: Person[] = [
      { id: 'p1', name: 'Alice', teamId: 't1', rotationOffset: 0, ineligible: [] },
      { id: 'p2', name: 'Bob', teamId: 't2', rotationOffset: 0, ineligible: [] },
      { id: 'p3', name: 'Charlie', teamId: UNASSIGNED_TEAM_ID, rotationOffset: 0, ineligible: [] },
    ]
  ...
  it('generates exact per-person CSV with 8.5h shift and cross-midnight shift, skipping OFF days', () => {
    ...
    const people: Person[] = [
      { id: 'p1', name: 'Dana', teamId: 't1', rotationOffset: 0, ineligible: [] },
    ]
  ...
  it('escapes fields with commas and double quotes correctly per RFC 4180', () => {
    const people: Person[] = [
      { id: 'p1', name: 'Smith, John', teamId: 't1', rotationOffset: 0, ineligible: [] },
      { id: 'p2', name: 'Jane "Doc" Doe', teamId: 't2', rotationOffset: 0, ineligible: [] },
    ]
  ...
  it('excludes soft-removed people from both grid and per-person exports', () => {
    const people: Person[] = [
      { id: 'p1', name: 'Alice', teamId: 't1', rotationOffset: 0, ineligible: [] },
      { id: 'p2', name: 'Removed Person', teamId: 't1', rotationOffset: 0, ineligible: [], removed: true },
    ]
