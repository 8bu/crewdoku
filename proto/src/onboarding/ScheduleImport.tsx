...
import { DEFAULT_SHIFTS, OFF_CODE, type Assignment, type BoardData, type Person, type ShiftDef, type Team } from '../board/mockBoard'
    ...
    <div className="flex flex-col gap-4">
      ...
      {preview && (
        <p className="text-xs text-base-content/50">
          {rowCount} {rowCount === 1 ? 'person' : 'people'} · {filledCount} filled {filledCount === 1 ? 'cell' : 'cells'} across{' '}
          {importedDates.length} {importedDates.length === 1 ? 'day' : 'days'}
        </p>
      )}

      <details className="rounded-lg border border-base-300 bg-base-200/40 px-3 py-2 text-xs text-base-content/60">
        <summary className="cursor-pointer select-none font-medium text-base-content/70">CSV format</summary>
    ...
    <div className="flex h-full flex-col">
      <div className="min-h-0 flex-1 overflow-y-auto">
        <div className="mx-auto max-w-[720px] p-8">
          <div className="mb-5">
            <h1 className="text-xl font-semibold tracking-tight text-base-content">Import a schedule</h1>
            <p className="mt-1 text-sm text-base-content/60">
              Fill this board with a schedule you already worked — one from a spreadsheet, or from before Crewdoku — instead of
