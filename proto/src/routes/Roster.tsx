...
import { DEFAULT_SHIFTS, UNASSIGNED_TEAM, UNASSIGNED_TEAM_ID } from '../board/mockBoard'
...
export function Roster() {
  ...
  if (!period) return <Stub title="Roster" tickets="16" />
  ...
  return (
    <section className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 items-center gap-3 px-4 py-2.5">
        <h1 className="m-0 text-sm font-semibold tracking-tight">Roster</h1>
        <span className="text-xs tabular-nums text-[color:var(--text-dim)]">
          {filtered ? `${rows.length} of ${active.length}` : active.length} {active.length === 1 ? 'person' : 'people'}
        </span>
        <div className="ml-auto flex items-center gap-2">
          ...
          <Select
            ...
            options={[
              ...
              { value: UNASSIGNED_TEAM_ID, label: UNASSIGNED_TEAM.name },
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {active.length === 0 ? (
          ...
          <div className="max-w-[880px] px-4 py-4">
            {rows.length === 0 && <p className="m-0 py-3.5 text-sm text-base-content/40">No one matches this filter.</p>}
            ...
            {teams.length === 0 && <p className="mt-3 text-sm text-base-content/40">No teams yet.</p>}
          </div>
        )}
      </div>
    </section>
