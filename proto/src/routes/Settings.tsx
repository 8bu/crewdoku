...
import { defaultCoverageTable, useCoverageRules, type CoverageBand } from '../state/coverageRules'
import { useSolveSettings, type HardRuleId, type SoftGoalId } from '../state/solveSettings'
...
import { DEFAULT_SHIFTS, type BoardData, type ShiftDef } from '../board/mockBoard'
...
export function Settings() {
  ...
  if (!period || !initial) return <Stub title="Settings" tickets="15" />
...
export function SettingsPage({ periodId, initial }: { periodId: string; initial: BoardData }) {
  ...
  function handleToggleHardRule(id: HardRuleId) {
    if (id === 'H4') return
  ...
  return (
    <section className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 items-center gap-3 px-4 py-2.5">
        <h1 className="m-0 text-sm font-semibold tracking-tight">Settings</h1>
        <span className="text-xs tabular-nums text-[color:var(--text-dim)]">
          {shifts.length} {shifts.length === 1 ? 'shift' : 'shifts'} · {hardRulesOn}/{hardRuleFlags.length} hard
          rules on
        </span>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto border-t border-base-300">
        <div className="flex max-w-[880px] flex-col gap-8 px-4 py-5">
          <section className="flex flex-col gap-3">
            <div>
              <h2 className="m-0 text-sm font-semibold tracking-tight text-base-content">Period</h2>
              <p className="m-0 mt-0.5 text-xs text-base-content/60">This period's name and date range.</p>
            </div>
            <div className="flex max-w-[520px] items-end gap-4">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-2xs font-semibold uppercase tracking-wide text-base-content/40">Start</label>
                <input
                  type="date"
                  value={period.start}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-2xs font-semibold uppercase tracking-wide text-base-content/40">End</label>
                <input
                  type="date"
                  value={period.end}
