...
import { UNASSIGNED_TEAM, UNASSIGNED_TEAM_ID, type Person } from '../board/mockBoard'
...
export function Teams() {
  ...
  if (!period) return <Stub title="Teams" tickets="19" />
...
function TeamsPage({ period }: { period: Period }) {
  ...
  function teamLabel(teamId: string) {
    return sidebarTeams.find((t) => t.id === teamId)?.name || '(unnamed)'
  }

  return (
    <section className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 items-center gap-3 px-4 py-2.5">
        <h1 className="m-0 text-sm font-semibold tracking-tight">Teams</h1>
        <span className="text-xs tabular-nums text-[color:var(--text-dim)]">
          {c.teams.length} {c.teams.length === 1 ? 'team' : 'teams'} · {c.activePeople.length}{' '}
          {c.activePeople.length === 1 ? 'person' : 'people'}
        </span>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[260px_1fr] border-t border-base-300">
            ...
            {sidebarTeams.map((team) => {
              ...
              return (
                <li key={team.id}>
                  ...
                  >
                    <span className="truncate">{team.name || '(unnamed)'}</span>
        ...
        <div className="min-h-0 overflow-y-auto">
          <div className="flex max-w-[640px] flex-col gap-5 px-5 py-4">
            {isUnassigned ? (
              <p className="text-lg font-semibold tracking-tight">Unassigned</p>
            ) : (
              <input
                type="text"
                value={selected.name}
            ...
            {!isUnassigned && (
              <>
                <div className="flex flex-col gap-2">
                  <h2 className="m-0 text-2xs font-semibold uppercase tracking-wide text-base-content/40">Wants</h2>
                ...
                <div className="flex flex-col gap-2">
                  <h2 className="m-0 text-2xs font-semibold uppercase tracking-wide text-base-content/40">Avoids</h2>
                  <div className="flex flex-wrap gap-1.5">
                    {c.shifts.map(({ code }) => (
                      <button
                        key={code}
            ...
            <div className="flex flex-col gap-2">
              ...
              {members.length === 0 ? (
                <p className="py-3.5 text-sm text-base-content/40">No one here.</p>
              ) : (
                  ...
                  {members.map((p) => (
                    ...
                    >
                      ...
                      <Select
                        ...
                        options={sidebarTeams.map((t) => ({ value: t.id, label: t.name || '(unnamed)' }))}
                      />
                    </li>
                  ))}
                </ul>
              ...
              {candidates.length > 0 ? (
                ...
                <p className="mt-1 text-xs text-base-content/40">Everyone active is already here.</p>
              )}
            </div>
            ...
            {!isUnassigned && (
              <div className="flex flex-col gap-2">
                <button
                  ...
                  title={c.teams.length <= 1 ? "Can't delete the last team" : undefined}
  ...
  return (
    <div className="relative mt-1 max-w-[420px]">
      <input
        ...
        placeholder={`Search ${candidates.length} ${candidates.length === 1 ? 'person' : 'people'} to add…`}
      ...
      {q && (
        <ul className="absolute inset-x-0 top-full z-10 mt-1 flex max-h-[260px] list-none flex-col gap-0.5 overflow-y-auto rounded-lg border border-base-300 bg-base-100 p-1.5 shadow-lg">
          {results.length === 0 ? (
            <li className="px-2 py-1.5 text-xs text-base-content/40">No match.</li>
          ) : (
            results.map((p) => (
              <li key={p.id}>
                <button
