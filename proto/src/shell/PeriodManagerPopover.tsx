  ...
  if (isConfirmingDelete) {
    return (
      <li className="flex items-center justify-between gap-2 rounded-md px-2.5 py-1.5">
        <span className="truncate text-sm text-base-content/60">Delete "{period.label}"?</span>
      ...
      >
        ...
        <div className="absolute right-1 top-1/2 flex -translate-y-1/2 gap-0.5 opacity-0 transition-opacity duration-150 group-focus-within:opacity-100 group-hover:opacity-100">
          ...
          <button
            ...
            disabled={isOnly}
            title={isOnly ? "The only period can't be deleted" : 'Delete period'}
...
function CreatePeriodForm({
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-sm font-semibold text-base-content">New period</p>
      ...
      <div className="flex flex-col gap-1">
        <label className="text-2xs font-semibold uppercase tracking-wide text-base-content/40">Starts</label>
        <input
          type="date"
          value={start}
          onChange={(e) => setStart(e.target.value)}
      ...
      <div className="flex flex-col gap-1">
        <label className="text-2xs font-semibold uppercase tracking-wide text-base-content/40">Length</label>
        <Select
          className="w-full"
          value={duration}
          onChange={(v) => setDuration(v as PeriodDuration)}
          options={PERIOD_DURATIONS.map((d) => ({ value: d.value, label: d.label }))}
        />
      </div>

      <div className="flex flex-col gap-0.5">
        <label className="text-2xs font-semibold uppercase tracking-wide text-base-content/40">Start with</label>
        <StartModeOption
          name="period-start-mode"
          label="New period"
          description="Starts empty; generate a schedule once it's created."
          checked={mode === 'ready'}
          onSelect={() => setMode('ready')}
        />
        <StartModeOption
          name="period-start-mode"
          label="Import an old schedule"
          description="Paste a CSV of an existing schedule on the next screen."
          checked={mode === 'import'}
