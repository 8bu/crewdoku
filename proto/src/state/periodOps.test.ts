...
import { coverageAtom, defaultCoverageTable } from './coverageRules'
import { DEFAULT_SOLVE_SETTINGS, solveSettingsAtom } from './solveSettings'
import { assignmentKey, type Assignment, type Person, type ShiftDef, type Team } from '../board/mockBoard'
...
describe('deletePeriodAtom', () => {
  const PERIOD_A: Period = { id: 'pa', label: 'Period A', start: '2026-10-01', end: '2026-10-14', seedMock: false, setup: 'ready' }
  const PERIOD_B: Period = { id: 'pb', label: 'Period B', start: '2026-10-15', end: '2026-10-28', seedMock: false, setup: 'ready' }
  ...
  it('selects the remaining period that starts latest when deleting selected period', () => {
    const PERIOD_C: Period = { id: 'pc', label: 'Period C', start: '2026-11-01', end: '2026-11-14', seedMock: false, setup: 'ready' }
    const store = createStore()
